...
impl Kernel {
    ...
    fn register_default_capabilities(&mut self) {
        ...
        let default_capabilities = vec![
            CapabilityMetadata {
                ...
                description: "Process and extract structured intents from natural language".to_string(),
    ...
    fn validate_dependency_acyclicity(&self, tasks: &[Task], dependencies: &[Dependency]) -> bool {
        ...
        for task_id in &task_ids {
            if !visited.contains(task_id) && has_cycle(task_id.as_str(), &adj, &mut visited, &mut rec_stack) {
                return false;
            }
        }
