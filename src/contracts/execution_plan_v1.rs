...
impl ExecutionPlanV1 {
    ...
    pub fn validate(&self) -> Result<(), PlanValidationError> {
        ...
        if self.goals.is_empty() {
            return Err(PlanValidationError("Goals collection cannot be empty".to_string()));
        }
        ...
        if self.tasks.is_empty() {
            return Err(PlanValidationError("Tasks collection cannot be empty".to_string()));
        }
        if self.gates.is_empty() {
            return Err(PlanValidationError("Gates collection cannot be empty".to_string()));
        }
