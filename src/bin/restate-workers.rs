...
pub trait ToolWorker {
    async fn execute_tool(tool_name: String, parameters: serde_json::Value) -> Result<serde_json::Value, anyhow::Error>;
}

// Gate Worker - evaluates a validation rule
#[restate_sdk::service]
pub trait GateWorker {
    async fn evaluate_rule(rule: String, context: serde_json::Value) -> Result<bool, anyhow::Error>;
}
...
impl ToolWorker for ToolWorkerImpl {
    async fn execute_tool(&self, tool_name: String, parameters: serde_json::Value) -> Result<serde_json::Value, anyhow::Error> {
        println!("Tool Worker executing tool: {} with parameters: {}", tool_name, parameters);
...
impl GateWorker for GateWorkerImpl {
    async fn evaluate_rule(&self, rule: String, context: serde_json::Value) -> Result<bool, anyhow::Error> {
        println!("Gate Worker evaluating rule: {} with context: {}", rule, context);
...
impl ExecutionWorkflow for ExecutionWorkflowImpl {
    async fn process_plan(&self, plan_json: String) -> Result<String, anyhow::Error> {
        ...
        let result = format!("Workflow completed for plan {} with {} tasks", plan.id, plan.tasks.len());
        Ok(result)
    }
}
